...
Class Actions extends DBConnection{
    ...
    function login(){
        extract($_POST);
        $username = isset($username) ? $this->escapeString($username) : '';
        $password = isset($password) ? $password : '';
        $sql = "SELECT * FROM admin_list where username = '{$username}' and `password` = '".md5($password)."' ";
        $result = @$this->query($sql);
        if(!$result){
            $resp['status'] = "failed";
            $resp['msg'] = "A database error occurred. ".$this->lastErrorMsg();
            return json_encode($resp);
        }
        $qry = $result->fetch_array(MYSQLI_BOTH);
        if(!$qry){
            $resp['status'] = "failed";
    ...
    function save_admin(){
        ...
        @$check= $this->query("SELECT count(admin_id) as `count` FROM admin_list where `username` = '{$username}' ".($id > 0 ? " and admin_id != '{$id}' " : ""))->fetch_array(MYSQLI_BOTH)['count'];
        if(@$check> 0){
            $resp['status'] = 'failed';
    ...
    function delete_admin(){
        ...
        @$delete = $this->query("DELETE FROM `admin_list` where admin_id = '{$id}'");
        if($delete){
            $resp['status']='success';
    ...
    function save_position(){
        ...
        $check = $this->query("SELECT count(position_id) as `count` FROM `position_list` where `position` = '{$position}' ".($id > 0 ? " and position_id != '{$id}'" : ""))->fetch_array(MYSQLI_BOTH)['count'];
        if($check >0){
            $resp['status']="failed";
    ...
    function save_purok(){
        ...
        $check = $this->query("SELECT count(purok_id) as `count` FROM `purok_list` where `purok` = '{$purok}' ".($id > 0 ? " and purok_id != '{$id}'" : ""))->fetch_array(MYSQLI_BOTH)['count'];
        if($check >0){
            $resp['status']="failed";
    ...
    function save_household(){
        ...
        $check = $this->query("SELECT count(household_id) as `count` FROM `household_list` where `house_no` = '{$house_no}' ".($id > 0 ? " and household_id != '{$id}'" : ""))->fetch_array(MYSQLI_BOTH)['count'];
        if($check >0){
            $resp['status']="failed";
